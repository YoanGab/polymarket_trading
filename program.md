...
6. **Run the baseline**: `uv run python scripts/train_model.py --model xgboost > run.log 2>&1`
7. **Record baseline metrics**: `grep "RESULT\|TEST_BRIER" run.log`
8. **Run baseline strategy eval**: `uv run python scripts/eval_strategies.py --forecast-mode ml_model --max-markets 500 --split val > eval.log 2>&1`
